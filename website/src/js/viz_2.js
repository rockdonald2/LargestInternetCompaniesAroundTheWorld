...
(function () {
    ...
    Viz.VIZUALIZATIONS[currViz] = function () {
        ...
        (function () {
            ...
            <label for="${k}Cat"><span id="${k.replaceAll(
                    ' ',
                    '_'
                )}Circle" class="viz_container--filter__checkbox--circle" style="background-color: ${
                    Viz.INDUSTRY_COLORS[k]
                }"></span> ${Viz.INDUSTRY_HU[k]}</label>
          </div>`;
            });
        ...
        const categories = document.querySelectorAll(
            ".viz_container--filter__checkbox input[type='checkbox']"
        );
        categories.forEach((k) =>
            k.addEventListener('change', function () {
                if (this.checked) {
                    document.querySelector(
                        `#${this.value.replaceAll(' ', '_')}Circle`
                    ).style.backgroundColor = Viz.INDUSTRY_COLORS[this.value];
                } else {
                    document.querySelector(
                        `#${this.value.replaceAll(' ', '_')}Circle`
                    ).style.backgroundColor = Viz.COLORS['grey'];
                }
                ...
                const initialData = data
                    .filterFunction(multivalue_filter(selectedCats))
                    .top(Infinity);
